...
$resp = Communicator::communicate(CommunicateURL::INVALIDATE_SESSION,["username" => $_GET["username"]]);

require_once(dirname(__DIR__,2)."/scripts/notifications/NotificationManager.inc.php");
$type = $resp["suc"] == 1 ? "s" : "e";
NotificationManager::addNewNotification("hr-invalidateSession-".$_GET["username"],"HR - Sesja użytkownika",$type,$resp["desc"]." (${_GET["username"]})");
